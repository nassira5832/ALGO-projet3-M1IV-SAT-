...
filename = "SAT_AVEC_SATLIB/resultatSAT2_ver_Sol.csv"
data = pd.read_csv(filename)

# Vérifiez les noms des colonnes
print(data.columns)

# Trier les données par le nombre de clauses, de variables et de littéraux si les colonnes existent
data_sorted_by_clauses = data.sort_values(by='nbr_clauses')
data_sorted_by_vars = data.sort_values(by='num_vars') if 'num_vars' in data.columns else data
data_sorted_by_literals = data.sort_values(by='num_literals') if 'num_literals' in data.columns else data
...
if 'num_literals' in data.columns:
