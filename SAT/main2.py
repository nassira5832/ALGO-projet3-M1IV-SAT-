...
filename = "SAT/resultatSAT2_ver_Sol.csv"
data = pd.read_csv(filename)

# Trier les données par le nombre de clauses, de variables et de littéraux
data_sorted_by_clauses = data.sort_values(by='num_clauses')
...
plt.plot(data_sorted_by_clauses['num_clauses'], data_sorted_by_clauses['temps'], label='Temps Moyen (s) en fonction du nombre de clauses', color='b', marker='o')
...
plt.plot(data_sorted_by_clauses['num_clauses'], data_sorted_by_clauses['memUsage'], label='Mémoire Utilisée (octets) en fonction du nombre de clauses', color='c', marker='^')
